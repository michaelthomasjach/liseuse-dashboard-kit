...
export const CHART_PROPS_REFERENCE: ChartPropSection[] = [
  ...
  },
  {
    ...
    props: [
      ...
      { name: "settingsOpen", type: "boolean", required: false, doc: "Controls the chart-settings modal from outside instead of it managing its own state — pairs with `onSettingsOpenChange` below, exactly as `isFullscreen`/`onFullscreenChange` do just above, and only takes effect when that callback is given. Normally the modal opens from inside the chart (double-click the symbol/chart-type label, or the plot's own right-click menu); a caller needs this when neither gesture is available, which is the mobile layout's case — `ChartWorkspace` opens the focused panel's settings from a button in its own bottom toolbar there, since a phone has no double-click and no right-click." },
      { name: "onSettingsOpenChange", type: "(open: boolean) => void", required: false, doc: "Fired whenever the chart-settings modal opens or closes, from any of the ways it can." },
