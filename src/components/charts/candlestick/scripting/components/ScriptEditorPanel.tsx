...
import { PlayIcon, PauseIcon, SaveIcon, RefreshIcon, PlusIcon, TrashIcon, EyeIcon, EyeOffIcon } from "../../../../icons";
...
/** The script editor's own host — a `Modal size="fullscreen"` (already portaled to
 *  `document.body`, see Modal.tsx's own doc, and already raised above every sibling panel the same
 *  way every other modal in this library now is) with a script-tab strip, a Run/Stop/Save/Reset/
 *  Format toolbar, the CodeMirror instance (lazy-loaded — see ScriptEditorCodeMirror.tsx's own
 *  doc), the "AVAILABLE INDICATORS" inspection list, and the last run's own error/console output.
 *
...
export function ScriptEditorPanel({
  ...
  return (
    <Modal open={open} onClose={onClose} title="Éditeur de script" size="fullscreen" footer={null}>
      <div className="lq-script-editor-panel">
          ...
          <>
            <div className="lq-script-editor-panel__toolbar">
              ...
              <button
                type="button"
                className="lq-script-editor-panel__toolbar-button"
                onClick={() => setFormatRequestId((n) => n + 1)}
              >
                Format
              </button>
