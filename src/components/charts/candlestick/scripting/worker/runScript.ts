...
export function runScript(snapshot: ScriptEngineSnapshot): ScriptRunResult {
  ...
  try {
    ...
    return { error: { message: err instanceof Error ? err.message : String(err) }, logs, plots: [], drawings: [], table: null, alerts: [] };
  }
  ...
  for (let i = 0; i <= snapshot.runUpToIndex; i++) {
    ...
    try {
      ...
      const { plots, drawings, table } = getPlotResult();
      return { error: toScriptError(err), logs, plots, drawings, table, alerts: getAlerts() };
    }
  }
  const { plots, drawings, table } = getPlotResult();
  return { error: null, logs, plots, drawings, table, alerts: getAlerts() };
}
