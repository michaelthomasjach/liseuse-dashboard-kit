...
/** One framed chart with its component name and a line saying what it is for. The name opens the
 *  chart's own documentation — the same contract as the sampler further down the page. */
function Frame({
  name,
  purpose,
  wide,
  ...
  return (
    <figure className={["lqx-chart-frame", wide && "lqx-chart-frame--wide"].filter(Boolean).join(" ")}>
      <div className="lqx-chart-frame__stage">{children}</div>
 ...
 *  ends with a hole in it. The two left out are CandlestickChart — it already has the
 *  hero to itself — and SeasonalityView, which only makes sense with a full candle history behind
 *  it and reads as an empty grid at this size. */
export function ChartGallery({ open }: { open: (path: string) => void }) {
  const [heatmapRef, heatmapWidth] = useMeasuredWidth();
  const [mapRef, mapWidth] = useMeasuredWidth();

  return (
    <div className="lqx-charts">
      <Frame name="LineAreaChart" purpose="Deux séries superposées, portefeuille en haut et indice en dessous" wide onOpen={() => open("/docs/charts-lineareachart--docs")}>
        <LineAreaChart
          height={230}
          area
      ...
      <Frame name="Heatmap" purpose="Capitalisation et variation par secteur" onOpen={() => open("/docs/charts-heatmap--docs")}>
        <div ref={heatmapRef} style={{ width: "100%" }}>
          {heatmapWidth > 0 && <Heatmap groups={SECTORS} width={heatmapWidth} height={230} />}
        </div>
      </Frame>

      <Frame name="DonutChart" purpose="Répartition, avec valeur au centre" onOpen={() => open("/docs/charts-donutchart--docs")}>
        <DonutChart data={SAMPLE_ALLOCATION} size={168} centerValue="48 %" centerCaption="Actions" />
      </Frame>

      <Frame name="BarChart" purpose="Barres colorées par signe" onOpen={() => open("/docs/charts-barchart--docs")}>
        <BarChart data={MONTHLY_RETURNS} colorByValue height={200} formatValue={(v) => `${v.toFixed(1)} %`} />
      </Frame>
      ...
      <Frame name="EarningsDotChart" purpose="Résultats attendus contre publiés" onOpen={() => open("/docs/charts-earningsdotchart--docs")}>
        <EarningsDotChart points={EARNINGS} fill />
      </Frame>

      <Frame name="WorldExposureMap" purpose="Exposition géographique du portefeuille" wide onOpen={() => open("/docs/charts-worldexposuremap--docs")}>
        <div ref={mapRef} style={{ width: "100%" }}>
          {mapWidth > 0 && <WorldExposureMap data={EXPOSURE} width={mapWidth} height={300} formatValue={(v) => `${v} %`} />}
        </div>
      </Frame>
    </div>
  );
